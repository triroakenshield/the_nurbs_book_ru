AFRAME.registerComponent("rsurf", {
  ...
  DrawSurf2: function (p, q, U, V, P, x, y) {
    ...
    for (var i = p; i < pnu; i++) {
      for (
        var nu = this.data.U[i] * ustep + 1;
        nu <= this.data.U[i + 1] * ustep;
        nu++
      ) {
        u = nu / ustep;
        res.push([]);
        for (var j = q; j < pnv; j++) {
          for (
            var nv = this.data.V[j] * vstep + 1;
            nv <= this.data.V[j + 1] * vstep;
            nv++
          ) {
            v = nv / vstep;
            if ((i >= x - 1) & (j >= y - 1)) {
  ...
  DrawAllSurf: function (hscale1, hscale2, p, q, U, V, P, x, y, group) {
    var ustep, vstep, uvertices, vvertices, rp;

    rp = this.DrawSurf2(p, q, U, V, P, x, y);
    ustep = rp.length;
    vstep = rp[0].length;
  ...
  init: function () {
    ...
    for (var j = 0; j < 4; j++) {
      var larr = this.DrawPath(3 + j, q, this.data.V, 3 - j, this.data.V[3 + j], this.data.V[4 + j]);
    ...
    this.DrawAllSurf(hscale1, hscale2, p, q, this.data.U, this.data.V, P, 4, 4, group
    );
