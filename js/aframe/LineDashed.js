AFRAME.registerComponent('linedashed', {
  schema: {
  ...
  init: function () {
    ...
    material = this.material = new THREE.LineDashedMaterial({ color: data.color, opacity: data.opacity, dashSize: 0.05, gapSize: 0.05, transparent: data.opacity < 1, visible: data.visible });
  ...
  update: function (oldData) {
    ...
    if (!isEqualVec3(data.start, oldData.start) || !isEqualVec3(data.end, oldData.end)) {
      positionArray[3] = (data.start.x + data.end.x)/2;
      positionArray[4] = (data.start.y + data.end.y)/2;
      positionArray[5] = (data.start.z + data.end.z)/2;
      geoNeedsUpdate = true;
    }

    if (!isEqualVec3(data.end, oldData.end)) {
      positionArray[6] = data.end.x;
      positionArray[7] = data.end.y;
      positionArray[8] = data.end.z;
      geoNeedsUpdate = true;
    }
